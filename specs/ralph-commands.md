...
판단 조건: `write_tasks "done" | grep -Ec '^\[ \]|^\[→\]'` == 0 (Legend 라인 필터 적용, 미완료 `[ ]` + 진행 중 `[→]` 카운트)
...
- **실행 전 질문**: plan 시작 전에 loop 자동 연결 여부 확인 (이것만 사용자 입력):
  ```
  plan 완료 후 loop를 자동으로 시작할까? (y/n)
  ```
- **사전 점검 + 실행을 단일 Bash 호출로 통합**: Claude Code 권한 프롬프트 최소화를 위해 사전 점검과 실행을 하나의 스크립트로 묶어 실행. 출력 형식:
  - `CONFLICT:<pid>` → 기존 프로세스 충돌. `kill <pid>` 후 재실행도 단일 Bash 호출로 처리
  - `MISSING:loop.sh` → 오류 출력 후 중단
  - `MISSING:PROMPT_plan.md` → 오류 출력 후 중단
  - `STARTED:<pid>` → 정상 시작
...
- **실행 후 안내**:
  ...
  ```
- **완료 감지**: `.ralph_pid` 프로세스가 종료될 때까지 **30초 간격** 폴링. **타임아웃 30분** — 초과 시 폴링 중단 + 로그 확인 안내. 종료 후 `.ralph_status`에 `Done.` 포함 여부로 정상 완료 판별.
- **완료 후 동작**:
  - 정상 완료 + 사전 질문에서 y 답변 → loop **사용자 개입 없이** 자동 시작. 사전 점검 2.0 (PID 충돌), 2.3 (IMPLEMENTATION_PLAN.md 확인 질문) 건너뛰고 바로 실행 단계 진입
  - 정상 완료 + 사전 질문에서 n 답변 → "loop 시작할까? (y/n)" 재질문
    - y → loop 시작 (사전 점검 2.0, 2.3 포함 전체 흐름)
    - n → 종료
...
- [ ] 사전 점검 + 실행이 단일 Bash 호출로 처리됨
- [ ] `CONFLICT` 시 kill + 재실행도 단일 Bash 호출로 처리됨
- [ ] `loop.sh` 없음 → 오류 + 안내
- [ ] `PROMPT_plan.md` 없음 → 오류 + 안내
- [ ] 출력이 현재 Claude 컨텍스트에 누적되지 않음 (로그 파일 리다이렉트)
- [ ] `.ralph_plan.log` 실행마다 생성/덮어씀
- [ ] 실행 전 "loop 자동 시작할까?" 질문 (유일한 사용자 입력)
- [ ] plan 완료 감지: 30초 간격 폴링, 30분 타임아웃
- [ ] `.ralph_status`에 `Done.` 확인으로 정상 완료 판별
- [ ] 정상 완료 + 사전 y → 사용자 개입 없이 loop 자동 시작
- [ ] 정상 완료 + 사전 n → loop 재질문 후 사용자 선택
- [ ] 비정상 종료 (`Done.` 없음) → loop 시작하지 않음
- [ ] plan→loop 전체 흐름에서 "loop 자동 시작할까?" 외 사용자 확인 프롬프트 없음
...
- **사전 점검 + 실행을 단일 Bash 호출로 통합**: Claude Code 권한 프롬프트 최소화. 출력 형식:
  - `CONFLICT:<pid>` → 기존 프로세스 충돌. `kill <pid>` 후 재실행도 단일 Bash 호출로 처리
  - `MISSING:loop.sh` → 오류 출력 후 중단
  - `MISSING:PROMPT_build.md` → 오류 출력 후 중단
  - `NO_PLAN` → IMPLEMENTATION_PLAN.md 없음. auto-chain 시 건너뜀, 직접 호출 시 경고 + 확인 (y/n)
  - `STARTED:<pid>` → 정상 시작
...
- [ ] `IMPLEMENTATION_PLAN.md` 없음 → 직접 호출 시 경고 + 확인 요청, auto-chain 시 건너뜀
...
### `loop.sh` — auto-termination이 Status Legend로 인해 동작 안 하는 버그

**Current Behavior**: 루프 종료 조건 체크 시 `IMPLEMENTATION_PLAN.md`에서 직접 `grep -Ec '^\s*- \[ \]|^\s*- \[→\]'`를 실행. Status Legend 섹션에 포함된 `- [ ] = not started`, `- [→] = in progress` 라인이 항상 매칭되어 `PENDING=2` 이상으로 잡힘. 실제 미완료 태스크가 0개여도 루프가 종료되지 않음.

**Expected Behavior**: auto-termination 체크 시 기존 `write_tasks()` 헬퍼를 재사용. `write_tasks "done"`은 이미 `<details>` 섹션 제외 + Legend 라인 필터(`if (line ~ /^=/) next`)를 처리하므로, 그 출력에서 `[ ]`/`[→]` 라인만 카운트.
...
# 변경 후 (fix)
PENDING=$(write_tasks "done" | grep -Ec '^\[ \]|^\[→\]' 2>/dev/null)
PENDING=${PENDING:-0}
```

`loop.sh`와 `dot-claude/ralph/loop-scripts.md` 템플릿 동시 수정.

**IMPLEMENTATION_PLAN.md 포맷 규칙** (고정):

Legend 라인은 반드시 `= ` 형식 사용:
```
- [ ] = not started
- [→] = in progress
- [x] = complete
...
`write_tasks()`의 `if (line ~ /^=/) next` 필터가 이 규칙에 의존. Legend 포맷 변경 시 필터도 함께 수정 필요.
이 포맷은 `PROMPT_build.md`와 `PROMPT_plan.md`에도 주석으로 명시.

**Regression Guard**: Legend 없는 정상 IMPLEMENTATION_PLAN.md에서 `write_tasks "done"` 출력은 동일하게 동작.

#### Acceptance Criteria

- [ ] 모든 태스크 완료 후 루프가 자동 종료됨 (`All tasks complete. Stopping loop.` 출력)
- [ ] Status Legend 라인이 PENDING 카운트에 포함되지 않음
- [ ] `PROMPT_build.md`, `PROMPT_plan.md`에 포맷 규칙 주석 추가됨
- [ ] `loop-scripts.md` loop.sh 템플릿도 동일하게 수정됨

# Spec: /ralph_plan & /ralph_loop Commands + loop.sh 동작
...
`/ralph_plan`, `/ralph_loop` 커맨드에서 loop.sh 실행 시 `unset CLAUDECODE` 선행:
...
- 로그 파일(`.ralph_plan.log` or `.ralph_loop.log`) 있으면 마지막 5줄 표시
- Ctrl+C로 종료
...
판단 조건: `grep -c '^\s*- \[ \]' IMPLEMENTATION_PLAN.md` == 0
...
- [ ] `/ralph_plan` (no args) → `./loop.sh plan 1` 실행
- [ ] `/ralph_plan 3` → `./loop.sh plan 3` 실행
...
- **사전 점검** (순서대로):
  ...
  4. `IMPLEMENTATION_PLAN.md` 없으면 → 경고 + `/ralph_plan` 먼저 실행 권장 (y/n)
...
- [ ] `/ralph_loop` (no args) → `./loop.sh 5` 실행
- [ ] `/ralph_loop 20` → `./loop.sh 20` 실행
- [ ] 실행 명령에 `unset CLAUDECODE &&` 포함
- [ ] `loop.sh` 없음 → 오류 + 안내
...
- 로그 파일 존재 시 마지막 5줄 표시 (`.ralph_plan.log` → `.ralph_loop.log` 순으로 탐색)
